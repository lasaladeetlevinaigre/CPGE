...
int hash_function(int b, unsigned int n_buckets, char *k)
{
  assert( k != NULL);
...
hashtbl *hashtbl_create(unsigned int n_buckets)
{
...
void hashtbl_add(hashtbl *ht, char * k, elt_type v)
{
  assert(ht != NULL);
  assert(k != NULL);
  int idx  = hash_function(B, ht->n_buckets, k);
...
hashtbl *hash_file(char *path, unsigned int n_buckets)
{
  assert(path != NULL);
  ...
  hashtbl *ht = hashtbl_create(n_buckets);// on choisit un nombre de seaux égal au nombre de mots du fichier pour aller vers une "pseudo-"injectivite
...
void test_hash_file(char *path, uint64_t n_buckets)
{
  ...
  hashtbl *ht = hash_file(path, n_buckets);
