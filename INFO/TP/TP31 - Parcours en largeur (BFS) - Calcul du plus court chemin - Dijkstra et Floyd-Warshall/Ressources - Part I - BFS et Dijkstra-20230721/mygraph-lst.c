...
// w est passé par adresse
bool wgraph_has_edge(wgraph *g, int u, int v, double *addr_w)
{
  fprintf(stderr, "wgraph_has_edge: ATTENTION:  pour les graphes par liste d'adjacence, cette primitive est en O(delta(u))...Avez vous choisi la bonne implémentation concrète de graphe??\n");
  ...
  if (list_find(g->adj[u], v, addr_w) == true)
    return true;
