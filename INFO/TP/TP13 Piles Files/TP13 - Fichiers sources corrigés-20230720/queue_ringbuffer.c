...
struct s_queue
{
  ...
  elt_type *tab; // tableau contenant les valeurs (séquentielles) de la queuee
};
...
queue *queue_create(int s_max)
{
  ...
  q->idx_peek = 0;
  q->siz_max = s_max;
  q->tab = malloc(s_max*sizeof(elt_type));
